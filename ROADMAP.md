...
> **As-built (2026-07-09):** the user directed a **ground-up rewrite** that exceeds this content-only scope — a governance-first IA, a reusable visual/diagram kit (new MDX components), a cited-standards page, and ~75 rewritten pages, grounded in live research of ~20 best-in-class doc sites. Landed & verified green; see `.claude/PHASE_STATE.md` Phase 8.5 close-out and plan `lets-kick-off-phase-parallel-dolphin.md`.
...
Phases 0–8.5 are complete (see `.claude/PHASE_STATE.md` for verified status). Phase 8.5 shipped a **ground-up documentation rewrite** — a governance-first IA, a 17-component reusable visual/diagram kit, ~75 tight pages (jargon defined-on-first-use, the $60k wire as the running example), a cited-standards page + compliance disclaimer, and `_redirects` for every moved URL — verified green (`next build` → 74 docs pages SSG; web vitest 7/7; tsc + eslint clean; 75/75 internal links resolve). Deploys automatically to Cloudflare Pages on push to `main`. **Next: Phase 9 — Marketing Site** — a bespoke scrollytelling landing that drives traffic into the improved `/docs`, reusing the Phase 7 demo embed + Cloudflare pipeline. Then **Phase 10 — Release Hardening**.
