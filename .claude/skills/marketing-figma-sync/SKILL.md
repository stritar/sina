...
- A skill can't fire on its own; the `broadsheet-figma-sync-reminder.mjs` PostToolUse hook nudges after a Broadsheet component `.tsx` edit (code → Figma half only — Figma edits raise no local file event, so run this manually after a Figma change).
