...
export function Matrix<R extends string, C extends string>({
  ...
  return (
    <section className="flex flex-col gap-3">
      <h2 className="font-mono text-xs uppercase tracking-wide text-text-subtle">{label}</h2>
      <div className="overflow-x-auto rounded-lg border border-subtle bg-surface p-4">
        <div
        ...
        >
          ...
          {cols.map((col) => (
            <div
              key={col.key}
              className="font-mono text-[11px] uppercase tracking-wide text-text-subtle"
            >
              {col.label}
            </div>
          ))}
          {rows.map((row) => (
            <Fragment key={row.key}>
              <div className="pr-2 font-mono text-[11px] uppercase tracking-wide text-text-subtle">
                {row.label}
              </div>
              {cols.map((col) => (
