...
// Theme colors, copied from packages/theme/theme.css so the icons match the site.
const INK_LIGHT = "#353b31"; // --sina-color-text (:root)
const INK_DARK = "#f5f6f4"; //  --sina-color-text (.dark)
...
await write(
  ...
  <style>
    /* Light theme (default): --sina-color-text from :root. */
    .mark { fill: ${INK_LIGHT}; }
    /* Dark theme: --sina-color-text from the .dark block. */
    @media (prefers-color-scheme: dark) { .mark { fill: ${INK_DARK}; } }
  </style>
  <g class="mark" transform="${place(32, 0.9)}"><path d="${SYMBOL_PATH}"/></g>
</svg>
`,
);

// 2. favicon.ico — the legacy fallback, and what a browser requests before it has
//    parsed any HTML. On an OPAQUE light plate: an .ico cannot respond to the
//    theme, and bare dark ink would vanish into a dark tab strip.
await write(
