 ...
 *  3. props sync — every English primitive page's `<PropsTable>` is fed by a
 *     `props/*.props.mjs` module, every module maps back to a page, every
 *     locale map has exactly the English prop keys, and English descriptions
 *     obey the no-dash prose rule (they left the `.mdx` files the docs-prose
 *     guard scans);
...
const LOCALES = ["es", "zh", "fr", "de", "ja"] as const;
...
describe("props data modules", () => {
  const pageSlugs = readdirSync(PAGES_DIR)
    .filter((f) => f.endsWith(".mdx") && !/\.(es|zh|fr|de|ja)\.mdx$/.test(f))
    .map((f) => f.replace(/\.mdx$/, ""))
  ...
  it("every English page with a <PropsTable> is fed by its props module, in every locale", () => {
    for (const slug of pageSlugs) {
      const english = readFileSync(join(PAGES_DIR, `${slug}.mdx`), "utf8");
      if (!english.includes("<PropsTable")) continue;
      expect(
        english,
      ...
      expect(english).toMatch(/<PropsTable rows=\{\w+PropRows\(\)\} \/>/);
      expect(moduleSlugs).toContain(slug);
      for (const locale of LOCALES) {
        const sibling = readFileSync(join(PAGES_DIR, `${slug}.${locale}.mdx`), "utf8");
        expect(
          sibling,
          `${slug}.${locale}.mdx must call the props helper with its locale`,
        ).toMatch(new RegExp(`<PropsTable rows=\\{\\w+PropRows\\("${locale}"\\)\\} />`));
      }
    }
  });
  ...
  it("locale maps have exactly the English prop keys; English descriptions have no dashes", async () => {
    for (const slug of moduleSlugs) {
      const mod = await import(join(PROPS_DIR, `${slug}.props.mjs`));
      const propDoc = Object.values(mod).find(
        (v): v is { props: Array<{ prop: string; description: string }>; i18n: Record<string, Record<string, string>> } =>
          typeof v === "object" && v !== null && "props" in v,
      );
      expect(propDoc, `${slug}.props.mjs exports no PropDoc`).toBeDefined();
      const englishKeys = propDoc!.props.map((r) => r.prop).sort();
      for (const locale of LOCALES) {
        expect(
          Object.keys(propDoc!.i18n[locale] ?? {}).sort(),
          `${slug}.props.mjs i18n.${locale} keys must match props[].prop`,
        ).toEqual(englishKeys);
      }
      for (const row of propDoc!.props) {
        expect(
          /[—–]/.test(row.description),
          `${slug}.props.mjs "${row.prop}": em/en-dash in English description (prose rule)`,
        ).toBe(false);
      }
    }
  });
