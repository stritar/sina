...
describe("redirects", () => {
  ...
  it("every redirect targets a live page (no chains)", () => {
    // Wildcard destinations (`/docs/:path*`, the retired-locale rules) and the
    // site root can't be checked against the docs page set; the same-slug
    // mapping keeps them chain-free by construction.
    const checkable = redirectRules.filter(
      (r) => r.destination !== "/" && !r.destination.includes(":path*"),
    );
    const dead = checkable.filter((r) => !liveUrls.has(r.destination));
