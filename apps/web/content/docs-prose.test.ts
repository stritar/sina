 ...
 * children and PropsTable descriptions), the marketing copy, and the chrome
 * catalog (app/components/docs/messages.ts, which holds the marketing tagline +
 * header/search/pager strings). Repo files (CLAUDE.md, ROADMAP.md, code
...
const EN_MESSAGES = join(APP_ROOT, "components", "docs", "messages.ts");
...
describe("docs prose — no em-dashes", () => {
  ...
  it("the marketing copy + English chrome catalog use no em-dash or en-dash", () => {
    ...
    const offenders = [
      ...
      ...dashLines(strip(readFileSync(EN_MESSAGES, "utf8")), "app/components/docs/messages.ts"),
    ];
    expect(offenders).toEqual([]);
  });
});
