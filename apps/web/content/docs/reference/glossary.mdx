...
| **Violation** | One thing the gate found wrong, with a message, a severity, and a cited standard. |
...
so another domain is [additive](/docs/guides/extend-sina), not a rewrite. None
is built yet.
...
Published at **0.1.0**, so it's early. The gate and primitives are tested
(including an adversarial suite and automated accessibility checks); the audit
trail is a defined contract with a no-op sink. Evaluate accordingly, and see
[Releases & stability](/docs/reference/changelog).
