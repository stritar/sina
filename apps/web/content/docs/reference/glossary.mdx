...
The shipped rules are fintech-only. The
[governance seam](/docs/reference/packages) is domain-agnostic, so
[another domain can be added](/docs/guides/extend-sina); none is built or
currently planned.
...
It's an early 0.x project: public APIs may change between minors. The gate and
primitives are tested (including an adversarial suite and automated
accessibility checks); the audit trail is a defined contract with a no-op
sink. Evaluate accordingly; current version and status live in
