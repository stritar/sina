 ...
 * optional dot or icon), never by color alone, so it stays accessible. A vivid
 * saturated fill with black ink across neutral + status intents, in two sizes.
 * Spacing rides on the label
...
import { Warning, WarningOctagon, X } from "@phosphor-icons/react/dist/ssr";
...
// Vivid fill: saturated bg + black ink.
...
// Mandatory filled glyph for the severity intents — meaning is never carried by
// tint + text alone (mirrors Alert/Toast). Caller may swap the glyph via `icon`
// but not remove it for danger/warning. See CLAUDE.md "Error/warning states
// carry a filled icon" + the /error-warning-icon skill.
const intentIcon: Partial<Record<BadgeIntent, PhosphorIcon>> = {
...
export interface BadgeProps extends Omit<HTMLAttributes<HTMLSpanElement>, "color"> {
  /** Vivid saturated fill across neutral + status intents. */
  intent?: BadgeIntent;
  /** Two sizes: sm reads as body text, md leans in. */
  size?: BadgeSize;
  /** Show a leading status dot (inherits the badge's text color). Superseded by
   * the mandatory filled glyph on `danger`/`warning` intents. */
  dot?: boolean;
  /** Leading icon (Phosphor glyph). Overrides the intent glyph / `dot`. `danger`
   * and `warning` always render a filled glyph — this swaps which one. */
  icon?: PhosphorIcon;
...
export function Badge({
  ...
  // Leading-slot resolution: explicit `icon` → mandatory intent glyph (danger/warning)
  // → `dot`. Severity intents always render a filled glyph, so the meaning never
  // rides on tint + text alone; `dot` stays available for the calmer intents.
  const LeadingIcon = Glyph ?? intentIcon[resolvedIntent];
  const showDot = dot && !LeadingIcon;
  ...
  return (
    ...
    >
      {showDot ? (
        ...
        // `weight="bold"` is required: a monoline stroke reads as a single tone in
        // the label color. A fill-weight glyph (e.g. ShieldCheck) has a knockout
        // whose cutout would show the pill background — reading as a different color
        // than the label. The glyph inherits that color via `.icon` (currentColor).
        <LeadingIcon aria-hidden weight="bold" className={cn(styles.icon, glyph)} />
      ) : null}
