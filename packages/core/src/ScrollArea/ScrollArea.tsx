...
>(({ className, children, ...props }, ref) => (
  <Primitive.Root ref={ref} className={cn("relative overflow-hidden", className)} {...props}>
    ...
    <Primitive.Scrollbar
      ...
      className="flex w-2.5 touch-none select-none p-0.5 transition-colors duration-fast ease-standard"
    >
      <Primitive.Thumb className="relative flex-1 rounded-full bg-border" />
    </Primitive.Scrollbar>
    <Primitive.Scrollbar
      orientation="horizontal"
      className="flex h-2.5 flex-col touch-none select-none p-0.5 transition-colors duration-fast ease-standard"
    >
      <Primitive.Thumb className="relative flex-1 rounded-full bg-border" />
    </Primitive.Scrollbar>
    <Primitive.Corner />
